...
Run bare in an interactive terminal it asks which complete artifact to load (newest first; incomplete ones are counted, not offered; a lone artifact is used and named) and which connection should receive it — the candidates are every configured connection that is not one of the artifact's sources. Non-interactively it takes the newest complete artifact and `database.default`, and names what to pass when that connection is a source. When every configured connection is a source there is nowhere safe to load into, so it prints the `config/database.php` block to add (in the driver of the artifact's first source) and exits 1.

Refuses outside `pull.allow_environments` (checked before the disk is touched), on a missing or incomplete (`status !== "complete"`) artifact, on an unknown target connection, on a target that is a source connection of the artifact (the refusal names the connections that can be targeted instead), when a target table is missing a column the artifact carries, or when the artifact holds the same bare table name from two connections. A table the artifact names but the target lacks is skipped and named, not refused. Replaces each named table inside its own transaction (delete, insert, verify the row count) with foreign-key checks off for the run (Postgres needs superuser or an equivalent role for `SET session_replication_role`); everything else on the target is untouched. Asks for confirmation unless `--force` or non-interactive, prints the summary, then runs `pull.after` hooks.
