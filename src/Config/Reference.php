...
final readonly class Reference
{
     ...
     */
    public static function fromArray(string|array $raw): self
    {
        if (is_string($raw)) {
            return self::fromTarget($raw, true, EdgeSource::Manual);
        }

        $target = $raw[0] ?? null;

        if (! is_string($target)) {
            throw new InvalidArgumentException('A reference must name its target as [table.column].');
        }
        ...
        return self::fromTarget(
            ...
            is_string($source) ? EdgeSource::from($source) : EdgeSource::Manual,
        );
    }

    private static function fromTarget(string $target, bool $descend, EdgeSource $source): self
    {
        ...
        return match (count($parts)) {
            ...
            default => throw new InvalidArgumentException("Reference target [$target] must be [table.column] or [connection.table.column]."),
        };
    }
