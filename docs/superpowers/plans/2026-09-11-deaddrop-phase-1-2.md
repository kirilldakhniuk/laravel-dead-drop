...
2. **Descend (BFS).** For each table in the frontier, find every *inbound* edge from a `data` table where `descend` is true and the edge is eligible. Select that table's primary keys where the edge column joins the frontier table's key table. Apply `window` (`>= $since`, only when the table has a window and `$since` is not null) and `exclude` (`whereRaw`). Add to the child's key set; enqueue the child only if the key set actually grew. Lookup tables are never descended into.
